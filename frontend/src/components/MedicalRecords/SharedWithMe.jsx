...
const SharedWithMe = () => {
  ...
  // Fetch shared with me items using the correct API
  const fetchSharedWithMe = async () => {
    if (!userId) return;
    
    setLoading(true);
    try {
      const response = await axios.get(`/api/v1/shared-with-me?userId=${userId}`);
  ...
  const navigateToFolder = (subfolderId, file_type) => {
    // For shared items, we might want to handle this differently
    // For now, let's just log the action
    console.log('Navigate to shared item:', subfolderId, file_type);
  };

  const navigateUp = () => {
    // For shared items at root level, this might not be needed
    console.log('Navigate up in shared items');
  };
  ...
  // Group files by the user who shared them
