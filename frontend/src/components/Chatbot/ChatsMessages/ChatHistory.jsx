...
const ChatHistory = ({ chats, setChats, onChatSelect, selectedChatId, isSmallScreen, setView }) => {
  ...
  // Debug logging to track selection state changes
  console.log('ChatHistory render - selectedChatId:', selectedChatId, 'chats count:', chats.length);

  const onClickCreateChat = async () => {
    // Check if userId is valid before creating chat
    ...
    if (chatName) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          handleSelectChat(newChat.id); // Use newChat.id consistently
        } else {
  ...
  // Load initial chats if parent chats array is empty
  useEffect(() => {
    const fetchChats = async () => {
      // Don't fetch if userId is null, undefined, or the string "null"
      if (!userId || userId === 'null' || userId === null || userId === undefined) {
      ...
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          setChats(data); // Use parent's setChats function
        } else {
    ...
    // Only fetch if parent chats array is empty (initial load)
    if (chats.length === 0 && userId) {
      fetchChats();
    } else if (chats.length > 0) {
      setIsLoading(false);
    }
  }, [userId, chats.length, setChats]); // Depend on chats.length instead of chats object
