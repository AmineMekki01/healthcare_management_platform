...
const SearchComponent = ({ onUserSelect, onSelectChat }) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axios.get(`/api/v1/search/${username}/${userId}`, {
        params: {
          userType: userType
        }
      });
  ...
  const createOrSelectChat = async (selectedUser) => {
    ...
    try {
      const response = await axios.get('/api/v1/findOrCreateChat', {
        params: {
      ...
      {users.length > 0 && (
        <SearchResults>
          {users.map((user, index) => (
            <UserResult key={user.user_id || index} onClick={() => handleSelect(user)}>
              <UserAvatar
